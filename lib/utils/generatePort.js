...
const generatePort = (name) =>{
  ...
  if(fileExists(`.${startPath}/${nameResource.toLowerCase()}/services/${nameCapital}Port.java`)){
    return console.log(`Ya existe un puerto con el nombre ${nameCapital}Port`.red);
  }
  fs.writeFileSync(`.${startPath}/${nameResource.toLowerCase()}/services/${nameCapital}Port.java`, port( summary,nameResource.toLowerCase(), namePort.toLowerCase() ));
  console.log(`se creó un puerto con nombre: ${nameResource.toLowerCase()}/services/${nameCapital}Port`.green);
};
