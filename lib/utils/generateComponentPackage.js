...
const { responseDto } = require('../template/component/services/dtos/responseDto');
const {  converterDto } = require('../template/component/services/dtos/converterDto');
...
const generateComponentPackage = (name)=>{
  ...
  fs.writeFileSync(`.${startPath}/${name}/services/dtos/Response${nameCapital}Dto.java`, responseDto( summary, name ));
  fs.writeFileSync(`.${startPath}/${name}/services/dtos/Converter${nameCapital}Dto.java`, converterDto( summary, name ));
  ...
  console.log(`A resource was created with the name: ${name}`.green);
};
